...
# 设置颜色条
cb = plt.colorbar()
cb.ax.set_ylabel('Relative Density of Points')

plt.title('ML Correction Energy (kJ/mol)')
# 设置轴标签
plt.ylabel("Predicted Energy")
plt.xlabel("Reference Energy")
# plt.ylabel("Force Field Energy (kJ/mol)")
# plt.xlabel("ωB97M-D3BJ (gCP) Energy (kJ/mol)")

# 设置坐标轴样式
plt.tick_params(axis='both', which='both', bottom=True, top=False, left=True, right=False, labelbottom=True, labelleft=True)

# 保存图表
plt.savefig('test_res.png', dpi=600, bbox_inches='tight')